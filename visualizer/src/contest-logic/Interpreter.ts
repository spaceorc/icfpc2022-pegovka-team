...
export class Interpreter {
  ...
  interpret(lineNumber: number, context: Canvas, instruction: Instruction): InterpreterResult {
    switch (instruction.typ) {
      ...
      case InstructionType.HorizontalCutInstructionType: {
        return this.horizontalCutCanvas(
          lineNumber,
          context,
          instruction as HorizontalCutInstruction
        );
      }
  ...
  colorCanvas(
    ...
    if (block.typ === BlockType.ComplexBlockType) {
      ...
      context.blocks.set(
        blockId,
        new SimpleBlock(actualBlock.id, actualBlock.bottomLeft, actualBlock.topRight, color)
      );
      return new InterpreterResult(context, cost);
    }
    // Processing Ends
  ...
  pointCutCanvas(
    line: number,
    context: Canvas,
    pointCutInstruction: PointCutInstruction
  ): InterpreterResult {
    // TypeCheck Starts
    ...
    if (!point.isStrictlyInside(block.bottomLeft, block.topRight)) {
      throw Error(
        `At ${line}, encountered: Point is out of the [${blockId}]! Block is from ${block.bottomLeft} to ${block.topRight}, point is at ${point}!`
      );
    }
    // TypeCheck Ends
    ...
    if (block.typ === BlockType.ComplexBlockType) {
      ...
      (block as ComplexBlock).subBlocks.forEach((subBlock) => {
        ...
        // Case 2
        if (
          subBlock.bottomLeft.px <= point.px &&
        ...
        // Case 4
        if (
          subBlock.bottomLeft.py <= point.py &&
          point.py <= subBlock.topRight.py &&
      ...
      const topRightBlock = new ComplexBlock(blockId + ".2", point, block.topRight, topRightBlocks);
  ...
  verticalCutCanvas(
    line: number,
    context: Canvas,
    verticalCutInstruction: VerticalCutInstruction
  ): InterpreterResult {
    // TypeCheck Starts
    ...
    if (!(block.bottomLeft.px <= lineNumber && lineNumber <= block.topRight.px)) {
      throw Error(
        `At ${line}, encountered: Line number is out of the [${blockId}]! Block is from ${block.bottomLeft} to ${block.topRight}, point is at ${lineNumber}!`
      );
    }
    // TypeCheck Ends
  ...
  horizontalCutCanvas(
    line: number,
    context: Canvas,
    horizontalCutInstruction: HorizontalCutInstruction
  ): InterpreterResult {
    // TypeCheck Starts
  ...
  mergeCanvas(
    ...
    // Processing Starts
    const bottomToTop =
      (block1.bottomLeft.py === block2.topRight.py ||
    ...
    const leftToRight =
      (block1.bottomLeft.px === block2.topRight.px ||
    ...
    if (!(bottomToTop || leftToRight)) {
      throw Error(
        `At ${line}, encountered: Blocks [${blockId1}] and [${blockId2}] are not mergable, check canvas: [${JSON.stringify(
          context
        )}]`
      );
    }
