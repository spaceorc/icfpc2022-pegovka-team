...
export class Parser {
  ...
  parseLine(lineNumber: number, line: string): ParseResult {
    ...
    const COLOR_INSTRUCTION_REGEX = new RegExp(
      `color\\\[\(${block_id_re}\)\\\]\\\[(${color_re})\\\]$`
    );
    const LINE_CUT_INSTRUCTION_REGEX = new RegExp(
      `cut\\\[\(${block_id_re}\)\\\]\\\[\(x|X|y|Y\)\\\]\\\[\(${number_re}\)\\\]$`
    );
    const POINT_CUT_INSTRUCTION_REGEX = new RegExp(
      `cut\\\[\(${block_id_re}\)\\\]\\\[\(${point_re}\)\\\]$`
    );
    const MERGE_INSTRUCTION_REGEX = new RegExp(
      `merge\\\[\(${block_id_re}\)\\\]\\\[\(${block_id_re}\)\\\]$`
    );
    const SWAP_INSTRUCTION_REGEX = new RegExp(
      `swap\\\[\(${block_id_re}\)\\\]\\\[\(${block_id_re}\)\\\]$`
    );
    ...
    if (colorMatchResult) {
      ...
      const color = new RGBA(
        colorMatchResult.slice(6).map((value) => +value) as [number, number, number, number]
      );
    ...
    if (lineCutMatchResult) {
      ...
      if (orientation === "x" || orientation === "X") {
        ...
        return {
          typ: "instruction",
          result: { typ, blockId, lineNumber } as VerticalCutInstruction,
        };
      } else {
        const typ = InstructionType.HorizontalCutInstructionType;
        return {
          typ: "instruction",
          result: { typ, blockId, lineNumber } as HorizontalCutInstruction,
        };
      }
    }
